...
Matrix4x4 Math::MkeScaleMatrix(const Vector3 &scale) {
  Matrix4x4 result = {scale.x, 0, 0,       0, 0, scale.y, 0, 0,
...
Matrix4x4 Math::MakeAffineMatrix(const Vector3 &scale, const Vector3 &rotate,
  ...
  Matrix4x4 rot =
      Multiply(Multiply(MakeRoteXMatrix(rotate.x), MakeRotateYMatrix(rotate.y)),
               MakeRotateZMatrix(rotate.z));
  Matrix4x4 result = Multiply(Multiply(MkeScaleMatrix(scale), rot),
...
Vector3 Math::Transform(const Vector3 &vector, const Matrix4x4 &matrix) {
  float x = vector.x * matrix.m[0][0] + vector.y * matrix.m[1][0] +
            vector.z * matrix.m[2][0] + matrix.m[3][0];
  float y = vector.x * matrix.m[0][1] + vector.y * matrix.m[1][1] +
            vector.z * matrix.m[2][1] + matrix.m[3][1];
  float z = vector.x * matrix.m[0][2] + vector.y * matrix.m[1][2] +
...
Matrix4x4 Math::MakeViewportMatrix(float width, float height) {
  ...
  result.m[1][1] = -height / 2.0f; // Y²‚Íã‰º‹t
  result.m[2][2] = 1.0f;
